<script lang="ts">
  ...
  function getGuideStatusColor(status: Guide['status']) {
    switch (status) {
      ...
      case 'busy': return 'bg-oak-500';
      case 'off': return 'bg-gray-400';
      case 'training': return 'bg-champagne-500';
      default: return 'bg-gray-400';
    }
  }
  ...
  function getTableStatusColor(status: Table['status']) {
    switch (status) {
      case 'available': return 'bg-vineyard-500';
      case 'reserved': return 'bg-oak-500';
  ...
  function getGuideStatusLabel(status: Guide['status']) {
    switch (status) {
      case 'available': return '在岗待命';
      case 'busy': return '已分配';
      case 'off': return '休息';
      case 'training': return '培训中';
  ...
  <div class="max-w-7xl mx-auto p-6">
    <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <Card class="p-5">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="font-display text-3xl font-bold text-wine-700 mt-1">{stats.total}</p>
          </div>
      ...
      <Card class="p-5">
        ...
        <p class="mt-3 text-sm text-gray-500">平均每组 {stats.total ? Math.round(stats.totalGuests / stats.total) : 0} 人</p>
      </Card>
      ...
      <Card class="p-5">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="font-display text-3xl font-bold text-vineyard-700 mt-1">{tStats.occupied + tStats.reserved}/{tables.length}</p>
          </div>
        ...
        <div class="mt-3 w-full h-2 bg-gray-200 rounded-full overflow-hidden">
          <div
            ...
            style="width: {((tStats.occupied + tStats.reserved) / tables.length) * 100}%"
          ></div>
        </div>
      </Card>
    </section>
            ...
            {:else}
              {#each bookings as booking, index}
                <div>
                  ...
                  >
                    <div class="flex items-start justify-between" role="button" tabindex="0" on:click={() => selectedBooking = selectedBooking?.id === booking.id ? null : booking} on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { selectedBooking = selectedBooking?.id === booking.id ? null : booking; } }}>
                      <div class="flex items-start gap-4">
                        <div class="w-12 h-12 rounded-full bg-cream-200 flex items-center justify-center flex-shrink-0">
                          <span class="text-xl">{booking.tourId?.includes('dinner') ? '🍽️' : booking.tourId?.includes('cellar') ? '🏛️' : booking.tourId?.includes('professional') ? '🍷' : '🌿'}</span>
                        </div>
                        <div class="flex-1 min-w-0">
                          <div class="flex items-center gap-2 mb-1">
                            <h3 class="font-display font-bold text-lg text-wine-800">{booking.contactInfo?.name || '未知'}</h3>
                          ...
                          <p class="text-gray-600 mb-2">{booking.tour?.name || '未知套餐'}</p>
                          <div class="flex flex-wrap items-center gap-4 text-sm text-gray-500">
                            ...
                            <span class="flex items-center gap-1">
                              <Clock class="w-4 h-4" />
                              {booking.timeSlotId ? formatTime(booking.timeSlotId.split('_').pop() || '') : '未安排'}
                            </span>
                            <span class="flex items-center gap-1">
                              <Users class="w-4 h-4" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            >
                              ...
                              {#each getAvailableTables(booking.guestCount) as tbl}
                                <option value={tbl.id}>桌号 {tbl.number} (容纳 {tbl.capacity} 人)</option>
                              {/each}
                            </select>
                          </div>
                          <div>
                            ...
                            <div class="flex gap-2">
                              ...
                              {:else if booking.status === 'confirmed'}
                                <Button size="sm" variant="primary" onclick={() => updateBookingStatus(booking.id, 'completed')}>
                                  完成
                                </Button>
                              {/if}
                            </div>
            ...
            <Card class="p-5">
              ...
              {#each bookings.filter(b => b.guestCount >= 8) as groupBooking}
                <div class="flex items-start gap-3 mb-3 pb-3 border-b border-cream-200 last:border-0 last:mb-0 last:pb-0">
                  ...
                  <div>
                    <p class="font-medium text-wine-800">{groupBooking.contactInfo?.name || '未知'} - {groupBooking.guestCount} 人团</p>
                    <p class="text-sm text-gray-500">{groupBooking.date}</p>
            ...
            <Card class="p-5">
              ...
              {#each bookings.filter(b => b.status === 'confirmed').slice(0, 3) as upcoming}
                <div class="flex items-center gap-3 mb-3 pb-3 border-b border-cream-200 last:border-0 last:mb-0 last:pb-0">
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="font-medium text-gray-800 truncate">{upcoming.contactInfo?.name || '未知'}</p>
                    <p class="text-sm text-gray-500">{upcoming.tour?.name || '未知套餐'}</p>
                  </div>
        ...
        {#each guides as guide, index}
          <div>
            <Card class="p-5">
              <div class="flex items-start gap-4">
                <img
                  src={guide.avatarUrl || `https://picsum.photos/seed/${guide.id}/100/100`}
                  alt={guide.name}
        ...
        {#each tables as table, index}
          <div>
            <Card class="p-4 text-center">
              <div class="w-16 h-16 mx-auto mb-3 rounded-full {table.status === 'available' ? 'bg-vineyard-100' : table.status === 'reserved' ? 'bg-oak-100' : table.status === 'occupied' ? 'bg-wine-100' : 'bg-champagne-100'} flex items-center justify-center">
                <UtensilsCrossed class="w-7 h-7 {table.status === 'available' ? 'text-vineyard-600' : table.status === 'reserved' ? 'text-oak-600' : table.status === 'occupied' ? 'text-wine-600' : 'text-champagne-600'}" />
              </div>
              ...
              <p class="text-xs text-gray-400 mb-3">{table.location}</p>
              ...
              {#each ['10:00', '12:00', '14:00', '16:00', '18:00'] as time}
                ...
                <div>
                  <div class="flex items-center justify-between mb-1">
                    ...
                    <span class="text-sm text-gray-500">{people}/{max} 人</span>
                  </div>
